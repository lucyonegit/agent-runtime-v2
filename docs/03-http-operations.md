...
所有非敏感运行策略统一放在 `src/config/runtime.json`，它也是默认值的唯一来源。
`src/config/runtime-config.ts` 统一暴露配置类型和加载入口，
`src/config/runtime-config.helper.ts` 只负责读取、环境变量覆盖与校验。
...
模型 token 能力、ReAct 上限、Context 压缩阈值、单次文件写入限制、Shell 超时、
常驻进程端口范围、Browser 超时和工具开关均从 `src/config/runtime.json` 读取。
文件遍历深度、输出截断、轮询间隔等实现与安全常量留在对应工具模块，不对部署层暴露。
构建时该文件会复制到 `dist/src/config/runtime.json`。也可通过
