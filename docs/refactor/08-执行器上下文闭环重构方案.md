...
### 9.4 ModelCall 可重放 Manifest

当前 AuditedChatModel 对同一 ReAct 中的后续模型调用复用初始 Manifest，无法描述局部列表中新追加的 AIMessage 和 ToolMessage。本次在现有 `input_manifest jsonb` 内增加版本化 replay 信息，不增加数据库列：

```ts
interface ContextReplayManifestV1 {
  schemaVersion: 1;
  orderedSources: Array<
    | { kind: 'fixed'; key: string; checksum: string }
    | { kind: 'summary'; summaryId: string }
    | { kind: 'message_group'; groupId: string }
    | { kind: 'runtime_output'; outputId: string }
    | { kind: 'tool_result'; toolCallId: string }
  >;
}
```

每一次真实模型 invoke/stream 都生成自己的 Manifest。旧记录保持可读；若旧 Manifest 信息不足且 checksum 无法验证，ModelCallContextLoader 返回不可重建错误。
...
- Plan Context 与 Step Context 聚合在 `planner/context`。
...
### 阶段 3：ReActExecutor 接管初始编译

- 为 ReActExecutor 增加 Context Source 输入。
- 启动或恢复时由 ReActExecutor 调用 ContextCompiler。
...
- 将 Direct Context、ToolExecutor、AuditedChatModel、AgentRunner 组装从 JobExecutionService 移走。
...
- 创建 PlanExecutor。
- StepExecutor 内部调用 StepContextLoader 和 ReActExecutor。
- 每个 StepRun 使用完整 Context。
- PlanFinalizer 只接收 PlanFinalInput。
...
### 阶段 8：逐 ModelCall Manifest

- 每次模型 invoke/stream 生成独立 replay manifest。
- 记录 ReAct 局部列表新增的 runtime output 与 tool result 来源。
- 增加 `AgentStore.getModelCall(modelCallId)` 等只读接口。
- 使用 inputChecksum 验证重建结果。
- 兼容旧 Manifest。

验收：同一 ReAct 的第二轮及后续 ModelCall 可以重建真实输入，不再复用第一轮 Manifest 假装准确。
...
| model_call inspect | 重建 checksum 与持久化 inputChecksum 一致 |
...
控制：新调用写 replay manifest 并校验 checksum；旧调用无法验证时明确返回不可重建，不伪造 exact 结果。
...
11. typecheck、unit tests、PostgreSQL integration tests 和 build 全部通过。
