...
实现结果：旧 `src/context/*` 与 purpose/scope 业务分派已移除；正式执行和只读检查统一使用 `runtime/loaders -> ContextMaterial -> compileContext`。冻结的 schema、事务命令、RuntimeEventWriter、AgentLoop、AgentRunner、StepRunner 与 PlanEngine 均未修改。最终验证为 66 个单元测试、24 个 PostgreSQL 集成测试、typecheck 与 production build 全部通过。
...
- 保持现有 `PlanEngine` 的 Plan 创建、读取与持久化链路。
- 创建、恢复和推进 StepRun。
- 将当前 StepRun 交给 `StepExecutor`。
- 所有 Step 完成后调用 `PlanFinalizer`。
- 保证 Plan Final 之后 Job 立即进入 completed，不再产生后续工作承诺。

`PlanContextLoader` 为 `StepContextLoader` 和 `ContextInspectionService` 提供统一的只读完整 Plan 材料；为遵守冻结边界，PlanExecutor 本身不替换 `PlanEngine` 现有的 Store 读取与写入流程。
...
当前实现中的 `PlanFinalizer` 是执行器边界包装，委托被冻结的 `PlanEngine.finalize()` 按原有顺序读取 Plan、PlanStep 与已提交的结构化 StepOutput，并完成最终消息事务。它不构建 Session Context、不读取未验证的 Step 草稿，也不依赖 `ContextPurpose`。
...
- `runtime/context` 聚合 Context 编译、材料、MessageGroup、TokenBudget 和压缩协调；其中 compiler 代码保持纯函数，不因目录位置而依赖执行器。
- `runtime/loaders` 聚合所有从持久化事实只读生成 ContextMaterial 的 Loader。
- `runtime/executors` 聚合 ReAct、Direct、Plan、Step 和 Plan Final 执行包装。
- `planner` 只承载规划算法、Prompt、StepOutput 解析及现有被冻结链路文件。现有 `plan-engine.ts` 与 `step-runner.ts` 本次不移动，避免目录调整触碰运行和落库链；新文件不得继续放入 `planner`。
- ContextInspectionService 是编排层单文件服务；ContextQuery 与 ContextSnapshot 类型就近定义，不再为三个类型拆目录。
- ModelCall Context 重建仍然是 Loader，因此与其他 Loader 一起放在 `runtime/loaders`。
...
- 将 `CONTEXT_RULES_VERSION` 移到 `context-compiler.ts`，值保持不变。
...
7. PlanFinalizer 不构建 Session Context，并通过冻结的 PlanEngine 只消费 Plan、PlanStep 与已验证 StepOutput。
