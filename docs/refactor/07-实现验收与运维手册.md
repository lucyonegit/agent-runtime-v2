...
- Context 业务选择由 `runtime/loaders` 负责，`ContextCompiler` 是无 Job/Plan/Step 分支的纯编译函数；Step Context 保留完整 Session 基线、Plan、历史 StepRun ReAct/tool 结果和 StepOutput。
- `ContextInspectionService` 提供 `next_turn | job | step_run | model_call` 四类只读重建；ModelCall 只有通过 InputManifest 与 inputChecksum 校验才标记为 exact。
- 模型消息、流式 chunk、工具定义、工具调用与 provider 统一使用 LangChain；Runtime 不维护平行 ModelPort/Tool DTO。
...
flowchart LR
  ...
  Executor --> RuntimeExecutors["Direct / Plan / Step / ReAct Executors"]
  RuntimeExecutors --> Context["Context Loaders / Compiler"]
  RuntimeExecutors --> Planner["PlanEngine / StepRunner"]
  RuntimeExecutors --> Loop["AgentLoop"]
  Loop --> Tools["ToolExecutor"]
  Executor --> Audit["AuditedChatModel"]
  Audit --> Model["LangChain BaseChatModel"]
  Model --> Provider["DashScope / OpenAI-compatible"]
...
| `DASHSCOPE_API_KEY` | DashScope 执行 Job 时 | 无 | 优先级高于 OPENAI_API_KEY；缺失凭据时服务仍可启动，Job 会 failed |
| `OPENAI_API_KEY` | 非 DashScope 执行 Job 时 | 无 | OpenAI-compatible fallback 凭据 |
| `OPENAI_MODEL` | 否 | DashScope 为 `qwen-plus`，其他为 `gpt-4.1-mini` | LangChain provider model id |
| `OPENAI_BASE_URL` | 否 | DashScope compatible-mode endpoint 或 provider 默认地址 | OpenAI-compatible 服务地址 |
| `PORT` | 否 | `3000` | HTTP 端口 |
...
Server、migration 和 reset 脚本均通过 `dotenv/config` 加载同一份本地配置；启动日志不得输出密钥或完整连接串。
