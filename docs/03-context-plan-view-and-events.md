...
每次模型调用前只经过 `ModelInputBuilder.buildForTask(Task, TaskRun)`：
...
flowchart TD
    ...
    G -->|是| I["MessageCompactor 压缩最旧完整消息组"]
    I --> J["覆盖 Session ContextCompaction"]
    J --> K["重新构建一次"]
    K --> L{"仍超限?"}
    L -->|否| H
    L -->|是| M["model_input_too_large"]
```
...
只有完整的旧 `conversation` 消息组可被压缩；当前 Task 目标和 Task scope 消息不压缩。每次超限最多做一次增量摘要：
...
- `tool_call.upserted` / `tool_run.upserted`：更新逻辑与物理执行状态。
